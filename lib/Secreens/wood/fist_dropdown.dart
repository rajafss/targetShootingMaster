...
class Fist extends StatelessWidget {
  const Fist({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return   StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<dynamic> types_wood = snapshot.data!.docs[0]['fist'];
...
class FistItem extends StatefulWidget {
  final List<String> list;
  const FistItem({Key? key, required this.list}) : super(key: key);

  @override
  State<FistItem> createState() => _FistItemState();
}

class _FistItemState extends State<FistItem> {
  String selectValue = '';
